...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder.MoCo_View(
        models.__dict__[args.arch],
        args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.topk, args.mlp, args.swap, args.negative)
    print(model)
    ...
    criterion = MultiNCELoss().cuda(args.gpu)
        ...
        if os.path.isfile(args.pretrain):
            ...
            for k,v in first_dict.items(): # only take the encoder_q
                if 'encoder_q.' in k:
                    ...
                    new_dict[k] = v
                    k = k.replace('encoder_k.', 'sampler.')
                    new_dict[k] = v
            first_dict = new_dict
            msg = model.load_state_dict(first_dict, strict=False)
    ...
    # Data loading code
    if args.dataset == "k400":
        traindir = os.path.join('/home/qianrui/dataset/kinetics-dataset/video_256', 'train')
        print(traindir)
...
def train(train_loader, augmentation_gpu, model, criterion, optimizer, epoch, args, writer):
    ...
    for i, (video, audio) in enumerate(train_loader):
        ...
        # compute output
        logit, label, caam = model(q, k)
        vv, vs, vd, sv, dv = logit
        label_vv, label_vs, label_vd = label
        qvc, ksdc = caam
        prob = nn.functional.softmax(vv.detach(), dim=1)
        vv = criterion(vv, label_vv)
        vs = criterion(vs, label_vs) + criterion(sv, label_vs)
        vd = criterion(vd, label_vd) + criterion(dv, label_vd)
        loss = vs + vd + vv + 0.1 * torch.pow(qvc-ksdc, 2).mean()
        gt = torch.zeros(prob.shape[0], dtype=torch.long).cuda()
        acc1, acc5 = accuracy(prob, gt, topk=(1, 5))
