...
class VideoResNet(nn.Module):
    ...
    def forward(self, x, wp=None, wn=None, train=True, weight=True):
        x = self.stem(x)
        ...
        if not train:
            activation = torch.sum(x, dim=1, keepdim=True)
            norm = torch.max(activation.view(*activation.shape[:2], -1), dim=-1)[0]
            activation = activation / norm.view(*norm.shape[:2], 1, 1, 1)
            return activation
